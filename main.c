...
int main(int argc, char** argv) {
    ...
    if (strcmp(argv[1], "--seg") == 0){
        ...
        if(strcmp(argv[1], "--XOR")==0){
            ...
            {
                read_file("source/Xor/Xor-weights.txt",number_of_inputs,number_of_outputs, \
                number_of_hidden_nodes,hidden_layer_bias,hidden_weights,output_layer_bias,output_weights);
            }
            else{
                initialization(number_of_inputs,number_of_hidden_nodes,number_of_outputs,\
            ...
            if (atoi(&answer[0])== 1)
            {
                ...
                for (int n=0; n < nb; n++)
                {
                    ...
                    {
                        i = forward_pass(x ,number_of_inputs,number_of_hidden_nodes,number_of_outputs,trainingSetOrder,\
                        training_inputs,hidden_weights,hidden_layer_bias,output_weights,output_layer_bias,hidden_layer,output_layer);
                        back_propagation(lr, i,number_of_inputs,number_of_hidden_nodes,number_of_outputs,training_inputs,\
                        training_outputs,hidden_weights,hidden_layer_bias,output_weights,output_layer_bias,\
                        hidden_layer,output_layer);
                    }
                    fprintf(result_file, "input : %f ^ %f => output = %f , expected : %f\n",training_inputs[i*number_of_inputs],\
                    training_inputs[i*number_of_inputs+1],output_layer[0],training_outputs[i*number_of_outputs]);
                }
                printf("\n");
                printf("\e[?25h");
                fclose(result_file);
