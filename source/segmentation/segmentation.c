    ...
    for (int i = 0; i < image->h; i++) {
        ...
        if (red == 0 || red == 255) {
            ...
            for (int i = 0; i < blocs[Count]->w; i++) {
                ...
                if (green == 128) {
                    chars[Count][CharCount] = SDL_CreateRGBSurface(SDL_HWSURFACE, bloc.h, bloc.h, 32, 0, 0, 0, 0);
                ...
                if (red == 0 || red == 255) {
                    ...
                    chars[Count][CharCount] = SDL_CreateRGBSurface(SDL_HWSURFACE, size, size, 32, 0, 0, 0, 0);
                    SDL_FillRect(chars[Count][CharCount],0,-1);
                    SDL_BlitSurface(blocs[Count], &chr, chars[Count][CharCount], &center);
...
int *Resize1(int *mat, int sizeFinalX, int sizeFinalY, int sizeX, int sizeY) {
	...
	for (int i = 0 ; i<sizeFinalY ; i++) {
		for (int j = 0 ; j<sizeFinalX ; j++) {
			res[i*sizeFinalX + j] = mat[((int)((double)i*coeffY))*sizeX + (int)((double)j*coeffX) + 1];
		}
	}
	return res;
}
...
int ImageToMatrix(SDL_Surface ***chars, int ***chars_matrix, int *len, int BlocNumber) {
    ...
    *chars_matrix = malloc(sizeof(int) * 5000);
	...
	for (int j = 0 ; j < BlocNumber; ++j) {
		for (int i = 0 ; i < len[j]; ++i) {
            (*chars_matrix)[count] = malloc(chars[j][i]->w * chars[j][i]->h * sizeof(int) + sizeof(int));
